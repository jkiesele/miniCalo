...
ToyModelHadronicProcess::ToyModelHadronicProcess(const G4String& processName) :
  G4VDiscreteProcess(processName), m_verboseLevel(0)
{
							...
							const G4Step& /*  step*/)
{
  ...
  const G4double phi_p = 2*pi*CLHEP::RandFlat::shoot()-pi ;
  const G4double theta_p = pi*CLHEP::RandFlat::shoot() ;
  ...
  if (numberOfSecondaries==2){
    ...
    CLHEP::HepRotation rotation(randomDirection,CLHEP::RandFlat::shoot()*2*pi);
